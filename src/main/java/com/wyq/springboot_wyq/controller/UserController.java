...
@RestController//作用：允许当前接口 UserController 的方法返回对象，且该对象转为Json文本  /查/:标注的类下的所有方法返回的数据直接写入HTTP响应体中
@RequestMapping("/user") //localhost:8080/user/**
public class UserController {
    //用不同的映射注解来实现增删改查
    //REST：HTTP协议中有不同的发生请求的方式，GET,POST,PUT,DELETE等，
    // 新增用户：http://localhost:8080/user POST
    // 删除用户：http://localhost:8080/user/1 DELETE
    ...
    // 增
    @PostMapping (value = "/user")  //前端请求的地址 -> URL:localhost:8088/user   对应的method:post
